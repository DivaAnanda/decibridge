...
def test_archive_transition_is_refused_when_integrity_fails(pilot_case, ketua_user):
    _force_locked(pilot_case)

    with pytest.raises(ValidationError) as exc:
        transition(
            pilot_case, "archive", ketua_user, reason="retensi", enforce_completeness=True
        )
...
def test_archive_still_works_without_the_opt_in_gate(pilot_case, ketua_user):
    """Internal flows and migrations must not be blocked by the gate."""
    _force_locked(pilot_case)

    transition(pilot_case, "archive", ketua_user, reason="retensi")
